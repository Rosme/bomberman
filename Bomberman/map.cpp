...
namespace DataModel {
	...
	Map::Map(boost::shared_ptr<Ressources::Manager> ressources)
		: m_array(boost::extents[27][21]), 
	...
	void Map::draw(sf::RenderTexture& renderTexture) {
		for(MapArray::index i = 0, iSize = m_array.size(); i != iSize; ++i) {
			for(MapArray::index j = 0, jSize = m_array[i].size(); j != jSize; ++j) {
	...
	DataModel::Player& Map::getPlayer(int index) {
