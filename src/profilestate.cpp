...
namespace States {
	...
	ProfileState::ProfileState(GameWindow& gameWindow, Options::Loader& profileLoader)
		: States::State(gameWindow), m_profileLoader(profileLoader), m_state(ProfileState::None), m_currentPage(0) {
			...
			newProfileLabel->connect(sf::Event::MouseButtonPressed, [this]() { setSubstate(ProfileState::New); });
			...
			nextLabel->connect(sf::Event::MouseButtonPressed, [this]() { loadNextPage(); });
			...
			previousLabel->connect(sf::Event::MouseButtonPressed, [this]() { loadPreviousPage(); });
			...
			m_newProfileEdit->connect(sf::Event::MouseButtonPressed, [this]() { m_newProfileEdit->setFullFocus(true); });
			...
			loadLabel->connect(sf::Event::MouseButtonPressed, [this]() { loadProfileFromLabel(); });
			...
			deleteLabel->connect(sf::Event::MouseButtonPressed, [this]() { deleteProfileFromLabel(); });
	...
	void ProfileState::loadProfilesLabel() {
		...
		for(std::size_t i = m_currentPage*5, j = 0; i < max; ++i) {
			...
			pName->connect(sf::Event::MouseButtonPressed, [this, &pName]() { loadFocusedSubstate(pName->getText()); });
	...
	std::size_t ProfileState::getMaximumPage() {
