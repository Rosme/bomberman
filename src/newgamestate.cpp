...
namespace States {
	...
	NewGameState::NewGameState(GameWindow& gameWindow, DataModel::MapLoader& mapLoader)
		: States::State(gameWindow), m_mapLoader(mapLoader), m_subState(new PlayingState(gameWindow, mapLoader, this)), m_playing(false), m_gen(m_device()) {
			...
			randomMapLabel->connect(sf::Event::MouseButtonPressed, [this]() { loadRandomMap(); });
	...
	void NewGameState::start() {
		...
		for(unsigned int i = 0; i < mapList.size(); ++i) {
			...
			map->connect(sf::Event::MouseButtonPressed, [this, map]() { startPlaying(map->getText()); });
	...
	void NewGameState::loadRandomMap() {
		auto mapList = m_mapLoader.loadMapList();
		std::shuffle(mapList.begin(), mapList.end(), m_gen);
	...
	PlayingState::PlayingState(GameWindow& gameWindow, DataModel::MapLoader& mapLoader, States::NewGameState* parent)
		...
		continueLabel->connect(sf::Event::MouseButtonPressed, [this]() { pauseGame(); });
		...
		quitLabel->connect(sf::Event::MouseButtonPressed, [this]() { m_parent->stopPlaying(); });
	...
	void PlayingState::verifyExplosionCollision(int level, sf::Vector2i startPosition, int owner) {
		std::discrete_distribution<> dist({ 0.6, 0.2, 0.2 });
